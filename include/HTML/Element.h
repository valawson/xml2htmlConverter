...
class Element {
public:
    explicit Element(const char* apName, const char* apContent = nullptr, const LocationType locationType = LocationType::Both) :
    mName(apName), mContent(apContent ? apContent : ""), mLocationType(locationType) {}
    Element(const char* apName, std::string&& aContent) :
        mName(apName), mContent(aContent), mLocationType(LocationType::Both) {}
    Element(const char* apName, const std::string& aContent) :
        mName(apName), mContent(aContent), mLocationType(LocationType::Both) {}
    Element(const char* apName, const LocationType locationType) :
        mName(apName), mContent(nullptr), mLocationType(locationType) {}
...
protected:
    ...
    std::ostream& toString(std::ostream& aStream, const size_t aIndentation = 0) const {
        if (mLocationType == LocationType::Start || mLocationType == LocationType::Both) {
            toStringOpen(aStream, aIndentation);
        }
        toStringContent(aStream, aIndentation);
        if (mLocationType == LocationType::End || mLocationType == LocationType::Both) {
...
class RowStart : public Element {
public:
    RowStart() : Element("tr", nullptr,  LocationType::Start) {}

    RowStart&& operator<<(Element&& aElement) = delete;
    RowStart&& operator<<(ColHeader&& aCol) {
        mChildren.push_back(std::move(aCol));
        return std::move(*this);
    }
    RowStart&& operator<<(Col&& aCol) {
        mChildren.push_back(std::move(aCol));
        return std::move(*this);
    }
    RowStart&& style(const std::string& aValue) {
        Element::style(aValue);
        return std::move(*this);
    }
};

class RowEnd : public Element {
public:
    RowEnd() : Element("tr", nullptr, LocationType::End) {}

    RowEnd&& operator<<(Element&& aElement) = delete;
    RowEnd&& operator<<(ColHeader&& aCol) {
        mChildren.push_back(std::move(aCol));
        return std::move(*this);
    }
    RowEnd&& operator<<(Col&& aCol) {
        mChildren.push_back(std::move(aCol));
        return std::move(*this);
    }
    RowEnd&& style(const std::string& aValue) {
        Element::style(aValue);
        return std::move(*this);
    }
};
...
public:
    TableStart() : Element("table", nullptr, LocationType::Start) {}

    TableStart&& operator<<(Element&& aElement) = delete;
    TableStart&& operator<<(Row&& aRow) {
        mChildren.push_back(std::move(aRow));
        return std::move(*this);
    }
    TableStart&& operator<<(Caption&& aCaption) {
        mChildren.push_back(std::move(aCaption));
        return std::move(*this);
    }
};
...
public:
    TableEnd() : Element("table", nullptr, LocationType::End) {}

    TableEnd&& operator<<(Element&& aElement) = delete;
    TableEnd&& operator<<(Row&& aRow) {
        mChildren.push_back(std::move(aRow));
        return std::move(*this);
    }
    TableEnd&& operator<<(Caption&& aCaption) {
        mChildren.push_back(std::move(aCaption));
        return std::move(*this);
    }
};
